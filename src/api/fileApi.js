...
export const fileApi = {
    fetchFiles: ({ userId }) => {
        return new Promise((resolve) => {
            setTimeout(() => {
                ...
                console.log("Check visible file: ", visibleFiles)
        ...
        return new Promise((resolve, reject) => {
            setTimeout(() => {
                ...
                if (fileIndex > -1) {
                    ...
                    delete mockFileDetails[fileId];
                    saveFiles()
                    console.log('API: File deleted successfully.');
                    resolve({ success: true, id: fileId });
                } else {
                    console.log('API: File not found for deletion.');
                    reject(new Error('Không tìm thấy file để xóa.'));
                }
            }, 500);
        });
    },
    ...
    uploadFile: (fileData) => {
        console.log('API: Uploading file:', fileData.name, 'to folder:', fileData.folderId);
        ...
        return new Promise((resolve) => {
            setTimeout(() => {
                const newFile = {
                    ...
                    name: fileData.name,
                    type: fileData.fileObject.type.split('/')[1] || 'unknown',
                ...
                const newFileDetails = {
                    ...
                    downloadCount: 0,
                    storageLocation: fileData.folderId,
                    collection: null,
                    status: 'On resource',
                    tags: [],
                    type: newFile.type,
                    ...
                    contentUrl: URL.createObjectURL(fileData.fileObject),
                };

                mockFiles = [newFile, ...mockFiles];
                mockFileDetails[newFile.id] = newFileDetails;
                saveFiles();

                console.log('API: File uploaded successfully:', newFile);
                resolve(newFile);
            }, 500);
        });
    },
